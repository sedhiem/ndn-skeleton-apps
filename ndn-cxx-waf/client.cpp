...
public:
  explicit Client(ndn::Face &face, const std::string &filename,
    ...
    : m_face(face)
    , m_baseName(
        ndn::Name("/my-local-prefix/simple-fetch/file").append(filename))
...
private:
  void sendInterest()
  {
    ...
    m_face.expressInterest(interest, std::bind(&Client::onData, this, _2),
  ...
  {
    std::cerr << "<< Data: " << std::string(reinterpret_cast<const char *>(
